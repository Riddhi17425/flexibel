    ...
    <div class="row align-items-center">
        <div class="border-0 mb-4">
            <div
                ...
                <h3 class="fw-bold mb-0">Edit Lifeimage</h3>
            </div>
        </div>
    </div>

    <div class="card-body">
        <form method="post" enctype="multipart/form-data" action="{{ route('lifeimage.update', $data->id) }}">
            @csrf
            @method('PUT')
                ...
                <div class="card mb-3 p-3">
                    ...
                    <div class="row g-3 align-items-center">
                        ...
                        <div class="col-md-12">
                            ...
                            <div id="imageInputs">
                                @php
                                    // Decode the existing images into an array
                                    $lifeimage = json_decode($data->lifeimage ?? '[]', true);
                                @endphp
                                ...
                                @if (!empty($lifeimage))
                                    @foreach ($lifeimage as $index => $item)
                                        <div class="input-group mb-2">
                                            <!-- Hidden input to preserve the existing image filename -->
                                            <input type="hidden" name="existing_lifeat_image[]" value="{{ is_array($item) ? $item['image'] : $item }}">
                                            
                                            <!-- Dropify file input -->
                                            <label for="image">Image</label>
                                            <input type="file" name="lifeimage[]" class="form-control dropify"
                                                data-default-file="{{ asset('public/lifeat_image/' . (is_array($item) ? $item['image'] : $item)) }}">
                                
                                            <!-- Alt text input -->
                                            <div class="col-md-10">
                                                <label for="alt_{{ $loop->index }}" class="form-label d-block">Alt Tag</label>
                                                <input type="text" id="alt_{{ $loop->index }}" name="existing_lifeimage_alt[]" class="form-control"
                                                    placeholder="Alt text"
                                                    value="{{ is_array($item) ? $item['alt'] ?? '' : '' }}">
                                            </div>
                            ...
                            <!-- Add More button to allow dynamic addition of file inputs -->
