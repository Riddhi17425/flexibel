<div class="modal fade" id="case_studies" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="case_studiesLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="case_studiesLabel">Know More About Us</h5>
           ...
           <form id="casestudyform" action="{{route('casestudy.submit')}}" method="post" enctype="multipart/form-data">
            ...
            <div class="row">
              ...
              <div class="col-md-12 mb-lg-4 mb-md-3 mb-4">
                <label>Name *</label>
                <input type="text" class="form-control" id="cs_fullname" name="fullname" maxlength="50"
                  oninput="this.value = this.value.replace(/[^a-zA-Z\s]/g, '').replace(/\s+/g, ' ').trimStart();"
                  placeholder="Enter your name">
                <small id="cs_fullnameError" class="text-danger"></small>
              </div>
              <div class="col-md-12 mb-lg-4 mb-md-3 mb-4">
                <label>Mobile Number *</label>
                <input type="text" class="form-control" id="cs_mobile" name="mobile" minlength="10" maxlength="15"
                  oninput="this.value = this.value.replace(/[^0-9]/g, '').slice(0, 15);" pattern="\d{10,15}"
                  title="Mobile number must be between 10 to 15 digits" placeholder="Enter your mobile number">
                <small id="cs_mobileError" class="text-danger"></small>
              </div>
              <div class="col-md-12 mb-lg-4 mb-md-3 mb-4">
                <label>Email Address *</label>
                <input type="email" id="cs_email" name="email" class="form-control" maxlength="60" placeholder="Enter your email address">
                <small id="cs_emailError" class="text-danger"></small>
              </div>
              ...
              <div class="col-md-12 mb-lg-4 mb-md-3 mb-4">
                <label>Message *</label>
                <!--<textarea class="form-control" rows="4" id="cs_message" name="message" maxlength="100" placeholder="Please describe your case study request"></textarea>-->
                <input type="text" class="form-control" rows="4" id="cs_message" name="message"  placeholder="Please share your experience with us">
                <small id="cs_messageError" class="text-danger"></small>
              </div>
              <div class="form-group mb-4">
                ...
                <small id="cs_recaptcha-error" class="text-danger"></small>
...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    ...
    const emailInput = document.getElementById('cs_email');
    const emailError = document.getElementById('cs_emailError');

    const mobileInput = document.getElementById('cs_mobile');
    const mobileError = document.getElementById('cs_mobileError');

    const fullnameInput = document.getElementById('cs_fullname');
    const fullnameError = document.getElementById('cs_fullnameError');

    const messageInput = document.getElementById('cs_message');
    const messageError = document.getElementById('cs_messageError');

    const countrySelect = document.querySelector('#country-select');
    const countryError = document.getElementById('cs_countryError');
    ...
    function validateRecaptcha() {
      ...
      const err = document.getElementById('cs_recaptcha-error');
      if (!token) {
    ...
    window.verifyCallback = function (response) {
      if (response && response.length > 0) {
        document.getElementById('cs_recaptcha-error').textContent = '';
      }
    };

    window.expireCallback = function () {
      document.getElementById('cs_recaptcha-error').textContent = 'Captcha expired. Please verify again.';
    };

    window.errorCallback = function () {
      document.getElementById('cs_recaptcha-error').textContent = 'Captcha error. Please reload and try again.';
    };
    ...
    if (form) {
      form.addEventListener('submit', function (e) {
        ...
        let valid = true;
        if (!validateField('cs_fullname', 'cs_fullnameError', 'Please enter your full name')) valid = false;
        if (!validateMobile()) valid = false;
        if (!validateEmail()) valid = false;
        if (!validateCountry()) valid = false;
        if (!validateField('cs_message', 'cs_messageError', 'Please enter your message')) valid = false;
      ...
      document.querySelectorAll('#casestudyform input, #casestudyform textarea, #casestudyform select').forEach(el => {
